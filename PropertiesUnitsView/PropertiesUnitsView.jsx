...
const PropertiesUnitsView = () => {
  ...
  const { fetchUnitId, UnitData, amenities } = UseGetHook("unit", id);
  const { FetchLeaseUnit, lease } = UseGetHook("leases", id);
  const { fetchTaskUnit, TaskData } = UseGetHook("tasks", id);
  const { FetchUnitTenant, TenantData } = UseGetHook("tenants", id);
  const { FetchUnitFile, fileData } = UseGetHook("files", id);
  const { FetchUnitNotes, NoteData } = UseGetHook("notes", id);
  useEffect(() => {
    fetchUnitId();
    FetchLeaseUnit();
    fetchTaskUnit();
  ...
  const data = lease.filter((e) => e.property.title.toLowerCase().includes(search.toLowerCase())
  )
    .map((e) => ({
      key: e.id,
  ...
  return (
    <>
      {EditUnit && (
        <AddUnitDetailsEditModal setUpdate={setUpdate} id={id} onClose={onCloseEditModalUnit} />
      )}
                  ...
                  <div className="row mt-3">
                    <div className="col-md-12">
                      <div className="task-search-property-container position-relative">
                        ...
                        <input
                          value={UnitData[0]?.property_title}
                          disabled
                          type="text"
                          className="form-control"
                        />
